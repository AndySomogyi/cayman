 ...
 *
 * Parses the top level expression, but wraps it up into a
 * anonymous function.
 */
 ...
 *
 * An external function definition, only a prototype.
 */
...
/**
 * Manages a list of function prototype definitions, and the LLVM context.
 *
 * The prototype definitions are never removed.
 */
...
/**
 * Exists for the durration of an IR generation call, manages a
 * single LLVM module into which the LLVM function and variable
 * defintions are generated.
 *
 * A LLVM module is a collection of the global variables, functions,
 * external references, and other data in LLVM. LLVM modules contain
 * LLVM IR definitions of the these functions and vars.
 *
...
: Session(S),
  M(new Module(GenerateUniqueName("jit_module_"),
	...
	/**
	 * remove the LLVM module, caller assumes owership
	 */
	...
	/**
	 * Gets an LLVM function prototype definition for a given function name.
	 * This name may be either a previously defined function, or an existing
...
public:
	...
	// The data type that references a set of LLVM modules which
	// are managed by the LazyEmitLayer.
	...
	/**
	 * Adds a complete function definition (with body).
	 *
	 * No IR generation is performed at this point.
	 */
	...
	ModuleHandleT addModule(std::unique_ptr<Module> M) {
		...
		// the LazyEmitLayer assumes ownersip of the given LLVM module.
		// once owned by the lazy emit layer, these modules may be
		// referenced via a ModuleHandle.
		return LazyEmitLayer.addModuleSet(singletonSet(std::move(M)),
				make_unique<SectionMemoryManager>(),
				std::move(Resolver));
	}

	/// Remove a module handle contained in the lazy emit layer.
	...
	/// Get the address of the compiled native code block for the
	/// given symbol name.
	/// Searches for a symbol in the lazy emit layer, this will perform
	/// JIT compilation if necessary, i.e. if the symbol has NOT been JITed
	/// already.
...
private:
	...
	// The compile layer object is used by lazy emit layer.
	CompileLayerT CompileLayer;

	// the only object code layer that user code interacts with,
	// the lazy emit layer manages a collection of LLVM modules, and
	// manages the IR to JIT compilation, and manages the symbol name to
	// address mappings.
...
/**
 * Evaluates a top level expression as an anonymous function
 *
 * The top level expression is parsed, and wrapped in an anonymous function.
 * IR is generated for this func, and stuffed into a new temporary LLVM module
 * which is then in turn stuffed into the JIT context, and JITed. The native
 * code func pointer is then called, and finally, the temp module is removed
