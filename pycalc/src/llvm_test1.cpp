/**
 * The kaleidoscope JIT demo from LLVM. 
 *
 * Here to experiment with the language, and to veryify that LLVM 
...
int llvm_test1_main(int argc, const char** argv)
{
	...
	std::string ErrStr;
	TheExecutionEngine =
			EngineBuilder(std::move(Owner)).setErrorStr(&ErrStr).setMCJITMemoryManager(
					llvm::make_unique<SectionMemoryManager>()).create();
