 ...
 */
CaAPI_DATA(void) CaType_Modified(CaType *type);
 ...
 */
CaAPI_DATA(int) CaType_IsSubtype(CaType *a, CaType *b);

/**
 * Return value: New reference.
 */
CaAPI_DATA(CaObject*) CaType_GenericAlloc(CaType *type, size_t nitems);

/**
 * Return value: New reference.
 */
CaAPI_DATA(CaObject*) CaType_GenericNew(CaType *type, CaObject *args,
		CaObject *kwds);


typedef enum {CA_VOID, CA_INT8, CA_INT16, CA_INT32, CA_INT64, CA_INT128, CA_FLOAT32,
	CA_FLOAT64, CA_STRING} CaPrimitive;

typedef enum {CA_POINTER = 1, CA_ARRAY = 1 << 1} Ca_PrimitiveModifier;


CaAPI_DATA(CaType*) CaType_FromPrimitive(uint32_t primitive,
